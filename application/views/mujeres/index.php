...
<div class="flex justify-center">
  <button type="button" class="bg-teal-600 px-5 py-3 rounded-md text-white hover:bg-teal-700 transition-all ease-in-out" data-bs-toggle="modal" data-bs-target="#modalAgregarPrenda">Agregar Prenda</button>
</div>

<!-- Modal Agregar -->
<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="modalAgregarPrenda" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    ...
    <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-gray-800 bg-clip-padding rounded-md outline-none text-current">
      <div class="modal-header flex flex-shrink-0 items-center justify-between p-4 border-b border-gray-200 rounded-t-md">
        <h5 class="flex justify-center text-xl text-center font-medium leading-normal text-white" id="exampleModalLabel">
          Agregar una nueva prenda de Mujer
        </h5>
        <button type="button" class="btn-close box-content w-4 h-4 p-1 text-black border-none rounded-none opacity-50 focus:shadow-none focus:outline-none focus:opacity-100 hover:text-black hover:opacity-75 hover:no-underline" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body relative p-4">
        <form action="<?php echo site_url('mujeres/guardarPrendaMujer'); ?>" method="post">
          <ul class="flex flex-col gap-3">
            <li class="flex flex-col justify-center">
              <label class="text-white" for="nombre">Nombre</label>
              <input class="p-2 rounded-md" type="text" name="nombre_pm" id="nombre" required>
            </li>
            <li class="flex flex-col justify-center">
              <label class="text-white" for="precio">Precio</label>
              <input class="p-2 rounded-md" type="text" name="precio_pm" id="precio" required>
            </li>
            <li class="flex flex-col justify-center">
              <label class="text-white" for="tipo">Tipo</label>
              <select class="p-2 rounded-md" id="tipo" name="tipo_pm" required>
                <option value="">- Seleccione una opción -</option>
                <option value="Pantalon">Camisa</option>
                <option value="Camiseta">Vestido</option>
                <option value="Camisa">Blusa</option>
                <option value="Chaqueta">Zapatos</option>
                <option value="Zapatos">Conjunto</option>
              </select>
            </li>
            <li class="flex flex-col justify-center">
              <label class="text-white" for="foto">Foto</label>
              <input class="p-2 rounded-md" type="text" name="foto_pm" id="foto">
            </li>
            <li class="flex flex-col justify-center">
              <label class="text-white" for="descripcion">Descripcion</label>
              <textarea class="p-2 rounded-md" type="text" name="descripcion_pm" id="descripcion"></textarea>
            </li>
          </ul>
  ...
  <?php foreach ($listaMujeres as $prendaMujer) : ?>
    <div class="flex">
      <div class="flex flex-col gap-1">
        ...
        <a href="<?php echo site_url('mujeres/cargarDatosPrenda') . '/' . $prendaMujer->id_pm ?>" aria-label="Editar Prenda" class="hint--left hint--rounded hover:scale-110 transition-all ease-in-out cursor-pointer z-10 translate-x-20 translate-y-6 relative flex justify-center items-center bg-teal-600 hover:bg-teal-700 w-12 h-12 rounded-full">
        ...
        <a href="<?php echo site_url('mujeres/eliminarMujer') . '/' . $prendaMujer->id_pm ?>" aria-label="Borrar Prenda" class="hint--left hint--rounded hover:scale-110 transition-all ease-in-out cursor-pointer z-10 translate-x-20 translate-y-6 relative flex justify-center items-center bg-red-600 hover:bg-red-700 w-12 h-12 rounded-full">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="absolute w-6 h-6 text-white">
